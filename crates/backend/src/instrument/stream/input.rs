    ...
    FProd: Send + 'static + FnOnce() -> Box<ProdType>,
{
    ...
    let handle = thread::spawn(move || {
        // Build audio consumer inside the owner thread.
        let prod = make_prod();
...
/// Build input stream across supported sample formats.
fn run_input(
    device: &cpal::Device,
    config: &StreamConfig,
    default_cfg: &SupportedStreamConfig,
    ...
    // Allocate scratch buffer with capacity in samples (frames * channels)
