...
use super::super::{Node, MODAL_EPSILON};

/// Map a linear mode index into a circumferential/radial-like pair.
///
/// Returns unitless integer orders used by path models.
pub(crate) fn mode_index_pair(index: usize) -> (usize, usize) {
    let mut i = index;
...
/// Radiation efficiency from ka and circumferential order.
///
/// - `ka` is dimensionless.
/// - Returns efficiency in [0, 1].
pub(crate) fn radiation_efficiency_from_ka(ka: f64, circumferential_order: usize) -> f64 {
...
/// Average displacement coupling focused around a target axial band.
///
/// Returns unitless coupling in [0, 1].
pub(crate) fn average_coupling(
    vertices: &[Point3<f64>],
...
/// Path-agnostic damping components used by strike/jet/slide formulas.
#[derive(Clone, Copy)]
pub(crate) struct DampingComponents {
    pub structural: f64,
    pub medium_viscous: f64,
    pub radiation: f64,
    pub clapper_coupling: f64,
    pub friction_drive: f64,
}

/// Compute shared damping components for a mode and medium.
///
/// All returned values are unitless and non-negative.
pub(crate) fn damping_components_for_medium(
    node: &Node,
    mode_index: usize,
    frequency_hz: f64,
    medium: &Medium,
    ...
    let friction_drive = node.clapper_to_bowl_friction.max(0.0)
