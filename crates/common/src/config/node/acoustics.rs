...
pub(super) fn strike_path_damping_for_medium(
    ...
    let (structural, medium_viscous, radiation, clapper_coupling) =
        damping_components_for_medium(node, mode_index, frequency_hz, medium, descriptor);
    (structural + medium_viscous + radiation * 0.02 + clapper_coupling * 1.6).clamp(1e-5, 0.95)
}
...
pub(super) fn jet_path_damping_for_medium(
    ...
    let (structural, medium_viscous, radiation, clapper_coupling) =
        damping_components_for_medium(node, mode_index, frequency_hz, medium, descriptor);
    (structural * 0.7 + medium_viscous * 1.4 + radiation * 0.06 + clapper_coupling * 0.2)
        .clamp(1e-5, 0.95)
}
