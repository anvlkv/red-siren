use cpal::{
    traits::DeviceTrait, Device, SupportedBufferSize, SupportedStreamConfig,
    SupportedStreamConfigRange,
};
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, Copy, PartialEq, PartialOrd, Eq, Serialize, Deserialize, ts_rs::TS)]
#[ts(export)]
pub enum PlaybackQuality {
    Auto(PlaybackQualityGate),
    LoFi,
    Medium,
    HiFi,
    Ultra,
...
impl Default for PlaybackQuality {
    fn default() -> Self {
        PlaybackQuality::Auto(PlaybackQualityGate::default())
    }
}

#[derive(
    Debug, Default, Clone, Copy, PartialEq, PartialOrd, Eq, Serialize, Deserialize, ts_rs::TS,
)]
...
impl From<SupportedStreamConfig> for PlaybackQualityGate {
    fn from(value: SupportedStreamConfig) -> PlaybackQualityGate {
...
impl PlaybackQualityGate {
    pub fn buffer_size(&self, cfg: Option<&SupportedStreamConfigRange>) -> u32 {
        ...
        let mapper = |c: &SupportedStreamConfigRange, default_buffer: u32| match c.buffer_size() {
            SupportedBufferSize::Range { min, max } => ultra_buffer.clamp(*min, *max),
            SupportedBufferSize::Unknown => default_buffer,
        };
    ...
    pub fn sample_rate(&self, cfg: Option<&SupportedStreamConfigRange>) -> u32 {
        return match self {
...
impl PlaybackQualityGate {
    pub fn select_output_config(&self, device: &Device) -> Option<SupportedStreamConfig> {
        device
    ...
    pub fn select_input_config(&self, device: &Device) -> Option<SupportedStreamConfig> {
        device
    ...
    fn is_a_better_than_b(
        ...
        let score = |bs: &SupportedBufferSize| -> i64 {
            match bs {
