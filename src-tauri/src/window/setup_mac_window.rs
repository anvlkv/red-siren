        ...
        if let raw_window_handle::RawWindowHandle::AppKit(appkit_handle) = handle.as_raw() {
            ...
            unsafe {
                let ns_view: Retained<NSView> =
                    Retained::retain(ns_view_ptr.cast()).ok_or_else(|| {
                        WindowError::MacOSSetupError(String::from("failed to retain NSView"))
                    })?;

                let ns_window = ns_view.window().ok_or_else(|| {
                    WindowError::MacOSSetupError(String::from(
                        "NSView was not installed in a window",
                    ))
                })?;
