...
use crate::audio_runtime::engine::AudioEngineError;
...
pub fn ensure_configured() -> Result<(), AudioEngineError> {
    ...
    AUDIO_SESSION_CONFIGURED.set(true).map_err(|_| {
        ...
        AudioEngineError::DeviceUnavailable
    })?;
...
#[cfg(target_os = "ios")]
unsafe fn configure_ios() -> Result<(), AudioEngineError> {
    ...
    if success != YES {
        ...
        return Err(AudioEngineError::DeviceUnavailable);
    }
