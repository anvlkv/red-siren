...
use common::error::{AppError, HealthError};
...
pub async fn health_grant_mic_premission(
    ...
    let check_result = if prompt {
        ...
        if supports_mic() {
            check_mic_permission().await.map_err(|e| {
                AppError::from(HealthError::MicPermissionCheckFailed {
...
fn maybe_toggle_windows(
    ...
    if state_lock.gui_ready {
        ...
        if let Some(splash_window) = app.get_webview_window("splashscreen") {
            splash_window.close().map_err(|e| HealthError::WindowOp {
                op: "close_splashscreen".into(),
                message: e.to_string(),
            })?;
        }

        if let Some(main_window) = app.get_webview_window("main") {
            main_window.show().map_err(|e| HealthError::WindowOp {
                op: "show_main".into(),
                message: e.to_string(),
            })?;
            ...
            main_window.set_focus().map_err(|e| HealthError::WindowOp {
                op: "focus_main".into(),
                message: e.to_string(),
            })?;
        }
    } else {
        log::debug!("GUI ready: {}.", state_lock.gui_ready,);
    }
