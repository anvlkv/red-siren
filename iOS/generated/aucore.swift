...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_aucore_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_aucore_rustbuffer_free(self, $0) }
    }
}
...
public class AuCoreBridge: AUCoreBridgeProtocol {
    ...
    deinit {
        try! rustCall { uniffi_aucore_fn_free_aucorebridge(pointer, $0) }
    }
...
fileprivate func uniffiInitContinuationCallback() {
    ffi_aucore_rust_future_continuation_callback_set(uniffiFutureContinuationCallback)
}

public func auLogInit()  {
    try! rustCall() {
    uniffi_aucore_fn_func_au_log_init($0)
}
}



public func new()  -> AuCoreBridge {
    return try!  FfiConverterTypeAUCoreBridge.lift(
        try! rustCall() {
    uniffi_aucore_fn_func_new($0)
}
    )
}

public func request(_ arcSelf: AuCoreBridge, _ bytes: Data) async  -> Data {
    return try!  await uniffiRustCallAsync(
        rustFutureFunc: {
            uniffi_aucore_fn_func_request(
                FfiConverterTypeAUCoreBridge.lower(arcSelf),
                FfiConverterData.lower(bytes)
            )
        },
        pollFunc: ffi_aucore_rust_future_poll_rust_buffer,
        completeFunc: ffi_aucore_rust_future_complete_rust_buffer,
        freeFunc: ffi_aucore_rust_future_free_rust_buffer,
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_aucore_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_aucore_checksum_func_au_log_init() != 64168) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_aucore_checksum_func_new() != 62064) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_aucore_checksum_func_request() != 8545) {
        return InitializationResult.apiChecksumMismatch
    }
